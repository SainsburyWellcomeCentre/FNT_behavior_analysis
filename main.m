...
% get list of animals to be analysed
Animal_IDs = para.Animal_IDs;

% Iterate though each animal in para.Animal_IDs
for mouseNum = 1:length(Animal_IDs) 

    Animal_ID = Animal_IDs(mouseNum);

    %% write intermediate variables and save locally
    
    % get list of files containing session-level behavioural data for each mouse
    filelist_behaviour = dir(fullfile(para.input_folder, Animal_ID,'**', ...
    ...
    for sessionNum=1:length(filelist_behaviour)
        ...
        if ~isfile(fullfile(output_folder_session, filename))
           ...
           trial_data_session = read_trial_data(fullfile(output_folder_session, ...
               filename));
        end
    ...
    if isfile(fullfile(para.output_folder, 'ephys_signal_quality', ...
            ['quality_metrics_KS3_metadata_', Animal_ID,'.csv']))
        ...
        warning(strcat("No ephys metadata found for animal ", Animal_ID, ...
            ". Skipping plot of signal quality."))
    end
    ...
    for sessionNum=1:length(filelist)
        ...
        trial_data_session = read_trial_data(fullfile(filelist(sessionNum).folder, ...
            filelist(sessionNum).name));
        Session_ID = trial_data_session.Session_ID(1,:);
    
        % plot accuracy, choice / abort trial bias, response times
        output_folder = fullfile(para.output_folder, 'plot_performance_across_trials', ...
            Animal_ID);
